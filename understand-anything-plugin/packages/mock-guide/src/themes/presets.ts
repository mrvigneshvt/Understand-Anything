...
// ponytail: as casts - arrays are non-empty by construction (PRESETS has 5 entries, each accentSwatches has 8)
//   upgrade: assert PRESETS.length > 0 at module level, replace `as` with indexed-access-non-null

export function getPreset(id: string): ThemePreset {
  return PRESETS.find((p) => p.id === id) ?? (PRESETS[0] as ThemePreset);
}
...
export function getAccent(preset: ThemePreset, accentId: string): AccentSwatch {
  return (
    ...
    (preset.accentSwatches[0] as AccentSwatch)
  );
}
